                    ...
                    {Object.entries(widget.supporting).map(([key, item]) => {
                        return (
                            <div key={key} className="w-full sm:w-1/2 p-12">
                                {
                                    key === 'created' || key === 'closed' || key === 'reOpened' ?
                                        <Typography
                                            className="text-12 font-semibold whitespace-nowrap"
                                            color="textSecondary"
                                        >
                                            {key === 'created' ? 'CPC' : null}
                                            {key === 'closed' ? 'CPM' : null}
                                            {key === 'reOpened' ? 'CTR' : null}
                                        </Typography>
                                        : null
                                }
                                {
                                    key === 'created' || key === 'closed' || key === 'reOpened' ?
                                        <Typography className="text-32 font-semibold tracking-tighter">
                                            {key === 'created' ? props.data?.cpc : '0'}
                                            {key === 'closed' ? props.data?.cpm : '0'}
                                            {key === 'reOpened' ? props.data?.ctr : '0'}
                                        </Typography>
                                        : null
                                }
