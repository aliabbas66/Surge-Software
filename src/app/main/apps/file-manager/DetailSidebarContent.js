...
function DetailSidebarContent(props) {
  ...
  const onDownloadItem = async () => {
    const auth = getAuth();
    await onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        await uploader.getDownloadURLforFiles(currentUser.uid, selectedItem.id);
        const downloadURL = uploader.fileURL;
        uploader.clear();
        if (downloadURL) {
    ...
    >
      {
        selectedItem.type !== 'image' ?
            ...
            <div>
              { url !== null ? <img src={url} alt="HA" /> : <div className="preview h-128 sm:h-256 file-icon flex items-center justify-center">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1, transition: { delay: 0.3 } }}>
                  <StyledIcon className="text-48" type={selectedItem.type} />
                </motion.div>
              </div>  }
