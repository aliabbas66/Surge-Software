...
export let adminEmail = 'aliabbas12e2@gmail.com';
...
const Frontend = props => {
                                ...
                                get(child(dbTwo, `users/${uid}/chats/${interlocutorUid}`)).then((snapshot) => {
                                    if (!snapshot.exists()) {
                                        set(ref(db, `users/${uid}/chats/${interlocutorUid}`), {
                                            ...
                                            newMsg: false,
                                            useFirst: false,
                                            lastMessage: uploader.fileType === 'msg' ? userMsg : 'file',
                                            time: msgBody.time
                                        }).then(() => {
                                        ...
                                        const ls = uploader.fileType === 'msg' ? userMsg : 'file';
                                        updateInterlocutorUser(interlocutorUid, true, ls, msgBody.time, uid);
                                        uploader.clear();
    ...
    const updateInterlocutorUser = async (interlocutorUid, msgCondition, lstMsg, tme, currentUserUID) => {
        const dbRef = ref(getDatabase());
        const db = getDatabase();
                                        ...
                                        <div className="right">
                                            <Form className="chatForm">
                                                <Form.Group className="mb-3 forms">{/*  controlId="formBasicEmail" */}
                                                    ...
                                                    <Form.Control id="pass" type="password" value= className="chat-input"/>*/}
